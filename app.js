...
function renderizarProdutos(cat) {
    ...
    if (filteredData.length === 0 && cat) {
        productContainer.innerHTML = `<p style="text-align: center; width: 100%; padding: 30px;">Nenhum produto encontrado para esta categoria.</p>`;
        return;
    }
...
function handleNewsletterSubscription(event) {
    ...
    if (!email) {
        mostrarFeedback("Atenção", "Por favor, insira um endereço de e-mail válido.", 'error');
        return;
    }
    ...
    if (newsletterEmails.includes(email)) {
        mostrarFeedback("Já Cadastrado", "Este e-mail já está inscrito em nossa newsletter. Obrigado!", 'error');
        emailInput.value = '';
        return;
    }
